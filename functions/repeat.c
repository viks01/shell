...
int repeat(char **token_ptr, char **prompt_ptr, char **cwd_ptr, char **prwd_ptr, char *home_path, char *command) {
    ...
    *token_ptr = strtok(NULL, " \t\n");
    ...
    while (n--) {
        ...
        if (strcmp(new_token, "pwd") == 0) {
            ...
            new_token = strtok(NULL, " \t\n");
            repeat(&new_token, prompt_ptr, cwd_ptr, prwd_ptr, home_path, new_command);
        } else {
            syscommands(&new_token, prog_name);
        }
